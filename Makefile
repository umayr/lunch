...
build: clean deps test
	@go build -x -ldflags ${LDFLAGS} -o bin/${BINARY} github.com/umayr/lunch/cmd/lunch
...
cross:
ifdef GOX
	@gox -ldflags=${LDFLAGS} -output="bin/{{.Dir}}_{{.OS}}_{{.Arch}}" -os=${OSES} -arch=${ARCHS} github.com/umayr/lunch/cmd/lunch
else
